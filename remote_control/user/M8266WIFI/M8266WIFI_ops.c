...
#define SPI_BaudRatePrescaler_256       ((uint32_t)0x000000FEU)
    ...
    M8266HostIf_SPI_SetSpeed(SPI_BAUDRATEPRESCALER_32);			// Setup SPI Clock. Here 200/8 = 25MHz for STM32F7xx, up to 40MHz
	...
	#endif
		 //spi_clk = 25000000;
		 spi_clk = 15000000;							// Setup SPI Clock. Here 480/32 = 15 for STM32H7xx, up to 40MHz
...
// u8 M8266WIFI_SPI_Config_AP(u8 ssid[13+1], u8 password[13+1], u8 enc, u8 channel, u8 saved, u16* status);
	if(M8266WIFI_SPI_Config_AP("Remote_CTL", "12345678", 4, 1, 0, &status)==0)  // set to 4=WPA_WPA2_PSK, not saved // 0=OPEN, 1=WEP, 2=WPA_PSK, 3=WPA2_PSK, 4=WPA_WPA2_PSK
    return 0;
}
#endif
...
//u8 M8266WIFI_SPI_Get_Opmode(u8* op_mode, u16* status);
	   ...
	   ||(sta_ap_mode == 3))  // if STA+AP mode(Chinese: 如果是STA+AP模式)
	{
		...
		//u8 M8266WIFI_SPI_STA_Connect_Ap(u8 ssid[32], u8 password[64], u8 saved, u8 timeout_in_s, u16* status);
 		     if(M8266WIFI_SPI_STA_Connect_Ap("Redmi", "1234567890", 0, 20, &status) == 0) // not saved, timeout=20s
				 return 0;
#endif
